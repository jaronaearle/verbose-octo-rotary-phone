// Jaron Earle
package computingtaxesjaronearle;

import java.util.Scanner;

public class ComputingTaxesJaronEarle
{

    public static void main(String[] args)
    {
        Scanner input;
        int status;
        double income;
        double tax;
        
        input = new Scanner(System.in);
        
        System.out.println("0 for Single\n1 for Married or qualified widow(er)");
        System.out.println("2 for Married filing separately\n3 for Head of Household");
        System.out.println("What is your filing status?: ");
        
        
        status = input.nextInt();
        
        System.out.println("Enter your taxable income: ");
        
        income = input.nextDouble();
        
        tax = 0;
        
        if (status == 0) 
        {
            if (income <= 8350) 
            {   
                tax = income * 0.10;
            }
            else if (income <= 33950)
            {
                tax = income * 0.10 + (income - 8350) * 0.15;
            }
            else if (income <= 82250)
            {
                tax = 8350 * 0.10 + (33950 - 8350) * 0.15 + (income - 33950) * 0.25;
            }
            else if (income <= 171550)
            {
                tax = 8350 * 0.10 + (33950 - 8350) * 0.15 + (82250 - 33950) * .025 + (income - 82250) * 0.28;
            }
            else if (income <= 372950) 
            {
                tax = 8350 * 0.10 + (33950 - 8350) * 0.15 + (82250 - 33950) * 0.25 + (171550 - 82250) * 0.28 + (income - 171550) * 0.33;
            }
            else 
            {
                tax = 8350 * 0.10 + (33950 - 8350) * 0.15 + (82250 - 33950) * 0.25 + (171550 - 82250) * 0.28 + (372950 - 171550) * 0.33 + (income - 372950) * 0.35;
            }
        }
        else if (status == 1) 
        {
            if (income <= 16700)
            {
                tax = income * 0.10;
            }
            else if (income <= 67900) 
            {
                tax = income * 0.10 + (income - 16700) * 0.15;
            }
            else if (income <= 137050) 
            {
                tax = 16700 * 0.10 + (67900 - 16700) * 0.15 + (income - 67900) * 0.25;
            }
            else if (income <= 208850) 
            {
                tax = 16700 * 0.10 + (67900 - 16700) * 0.15 + (137050 - 67900) * 0.25 + (income - 137050) * 0.28;
            }
            else if (income <= 372950) 
            {
                tax = 16700 * 0.10 + (67900 - 16700) * 0.15 + (137050 - 67900) * 0.25 + (208850 - 137050) * 0.28 + (income - 208850) * 0.33;
            }
            else 
            {
                tax = 16700 * 0.10 + (67900 - 16700) * 0.15 + (137050 - 67900) * 0.25 + (208850 - 137050) * 0.28 + (372950 - 208850) * 0.33 + (income - 372950) * 0.35;
            }
        }
        else if (status == 2) 
        {
            if (income <= 8350) 
            {
                tax = income * 0.10;
            }
            else if (income <= 33950)
            {
                tax = income * 0.10 + (income - 8350) * 0.15;
            }
            else if (income <= 68525) 
            {
                tax = 8350 * 0.10 + (33950 - 8350) * 0.15 + (income - 33950) * 0.25;
            }
            else if (income <= 104425) 
            {
                tax = 8350 * 0.10 + (33950 - 8350) * 0.15 + (68525 - 33950) * 0.25 + (income - 68525) * 0.28;
            }
            else if (income <= 186475) 
            {
                tax = 8350 * 0.10 + (33950 - 8350) * 0.15 + (68525 - 33950) * 0.25 + (104425 - 68525) * 0.28 + (income - 104425) * 0.33;
            }
            else 
            {
                tax = 8350 * 0.10 + (33950 - 8350) * 0.25 + (68525 - 33950) * 0.25 + (104425 - 68525) * 0.28 + (186475 - 104425) * 0.33 + (income - 186475) * 0.35;
            }
        }
        else if (status == 3)
        {
            if (income <= 11950) 
            {
                tax = income * 0.10;
            }
            else if (income <= 45500) 
            {
                tax = income * 0.10 + (income - 11950) * 0.15;
            }
            else if (income <= 117450) 
            {
                tax = 11950 * 0.10 + (45500 - 11950) * 0.15 + (income - 45500) * 0.25;
            }
            else if (income <= 190200) 
            {
                tax = 11950 * 0.10 + (45500 - 11950) * 0.15 + (117450 - 45500) * 0.25 + (income - 117450) * 0.28;
            }
            else if (income <= 372950) 
            {
                tax = 11950 * 0.10 + (45500 - 11950) * 0.15 + (177450 - 45500) * 0.25 + (190200 - 177450) * 0.28 + (income - 190200) * 0.33;
            }
            else 
            {
                tax = 11950 * 0.10 + (45500 - 11950) * 0.15 + (177450 - 45500) * 0.25 + (190200 - 177450) * 0.28 + (372950 - 190200) * 0.35;
            }
        }
        else
        {
            System.out.println("Error: invalid status");
            System.exit(1);
        }
        
        System.out.println("The tax is: " + (int)(tax * 100)/100.0);
    }
}

/*
0, 8000
0, 33000
0, 82000
0, 17100
0, 372000
0, 375000
1. 16500
1, 67000
1, 137000
1, 208000
1, 372000
1, 375000
2, 8000
2, 33000
2, 68000
2, 104000
2, 186000
2, 187000
3, 11900
3, 45500
3, 117000
3, 190200
3, 372000
3, 375000
4, 67000
*/
